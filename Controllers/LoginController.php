...
class LoginController
{
    ...
    function login_quenmk()
    {
        ...
        $this->login_model->login_quenmk($data);
    }
    function dangky()
    {
        $check1 = 0;
    ...
    function update()
    {
        ...
        if (isset($_POST['Ho'])) {
            ...
            if ($_POST['MatKhauMoi'] == $_POST['MatKhauXN']) {
                if (sha1($_POST['MatKhau']) == $_SESSION['login']['MatKhau']) {
                    $data = array(
                        'MatKhau' => sha1($_POST['MatKhauMoi']),
                    );
                    $this->login_model->update_account($data);
                } else {
                    setcookie('doimk', 'Mật khẩu cũ không đúng', time() + 2);
                }
            } else {
