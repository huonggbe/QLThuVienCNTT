<?php if (isset($_SESSION['isLogin_Admin']) && $_SESSION['isLogin_Admin'] == true || isset($_SESSION['isLogin_AdminTong']) && $_SESSION['isLogin_AdminTong'] == true) { ?>
  <a href="?mod=loaitailieu&act=add" type="button" class="btn btn-primary">Thêm mới</a>
<?php } ?>
...
<table class="table table-bordered" id="dataTable" width="100%" cellspacing="0">
  ...
  <tbody>
    <?php foreach ($data as $row) { ?>
      <tr>
        <td><?= $row['MaTaiLieu'] ?></td>
        <td><?= $row['TenLoaiTaiLieu'] ?></td>
        <td>
          <img src="../public/img/company/<?= $row['HinhAnh'] ?>" height="60px">
        </td>
        <td><?= $row['Mota'] ?></td>
        <td>
          <a href="?mod=loaitailieu&act=detail&id=<?= $row['MaTaiLieu'] ?>" class="btn btn-success">Xem</a>
          <?php if (isset($_SESSION['isLogin_Admin']) && $_SESSION['isLogin_Admin'] == true || isset($_SESSION['isLogin_AdminTong']) && $_SESSION['isLogin_AdminTong'] == true) { ?>
            <a href="?mod=loaitailieu&act=edit&id=<?= $row['MaTaiLieu'] ?>" class="btn btn-warning">Sửa</a>
            <a href="?mod=loaitailieu&act=delete&id=<?= $row['MaTaiLieu'] ?>" onclick="return confirm('Bạn có thật sự muốn xóa ?');" type="button" class="btn btn-danger">Xóa</a>
          <?php } ?>
        </td>
      </tr>
    <?php } ?>
